...
[`keifu`（系譜）](https://github.com/trasta298/keifu) は Git コミットグラフを静かに眺める TUI で、`sus4` のメンタルモデル上の親戚にあたる。**機能的に連携するわけではない** — 同じ「書かない人のための観察ツール」という思想を共有するだけだ。`keifu`（漢字）と `sus4`（音楽記号）の対比が、思想的な系列を作っている。

## Mental Model: alongside keifu

`sus4` は `keifu` と **連携を実装するつもりはない**。だが両者は「書かない人のための観察ツール」という同じ哲学を別領域に当てた姉妹で、思想的にどちらも同じテーブルに置くと収まりが良い：

|ツール|観察対象|
|---|---|
|`keifu`|コミットグラフ（「いまどのブランチ」「どこから分岐」）|
|`sus4`|ファイル / コード / diff の中身（「このファイル何が書いてある」「この変更何している」）|

ユーザーが手で `keifu → sus4` の順にコマンドを叩くのは自然な流れだが、それはあくまで人間側の動線であって、ツール側で連動するわけではない。
